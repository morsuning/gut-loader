...
- 🧰 **多数据库统一适配**：MySQL / PostgreSQL / openGauss / TXSQL / TDSQL / GaussDB / Oracle，达梦 DM 仅在 Windows x64 / Linux x64 / Linux arm64 可用
...
- 🎨 **现代桌面 UI**：Tauri v2 + React 18 + TypeScript + shadcn/ui，5 步线性向导，暖奶油 + 珊瑚色 Claude 品牌风格
...
| 商业数据库 | Oracle（oracle-rs 纯 Rust TNS 驱动）、达梦 DM（仅 Windows x64 / Linux x64 / Linux arm64） |
...
| 平台 | 目标路径 |
| --- | --- |
| Linux x64 | `src-tauri/bundled-drivers/dm-odbc/linux/x64/libdmodbc.so` |
| Linux arm64 | `src-tauri/bundled-drivers/dm-odbc/linux/arm64/libdmodbc.so` |
| Windows x64 | `src-tauri/bundled-drivers/dm-odbc/windows/x64/dmodbc.dll` |
...
| 达梦 DM | ODBC | 5236 | 仅限 Windows x64 / Linux x64 / Linux arm64 |
