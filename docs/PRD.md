...
- 现代化桌面 UI：Tauri v2 + React 18 + shadcn/ui，5 步线性向导，暖奶油 + 珊瑚色品牌风格（遵循 DESIGN.md 规范）
...
跨平台获取磁盘可用空间：macOS / Linux 通过 `df -k` 命令，Windows 通过 `GetDiskFreeSpaceExW` API。当目标路径不存在时，回退至父目录执行检查。
...
| 02 | 配置目标数据库 | 多类型数据库选择、参数表单、测试连接、LLM 智能填充、调试日志面板 |
...
- 当 API URL 或模型未配置时，「配置 LLM」按钮右上角显示琥珀色小圆点作为状态指示
- 弹窗内提供独立的 LLM 连接测试按钮
...
数据库配置页底部提供可折叠的调试日志面板，用于排查连接失败和应用运行时错误：

- 面板入口位于 LLM 智能识别面板下方，使用 Bug 图标与灰色低调文字标识，默认折叠不显眼
- 折叠状态下，若存在未查看的连接错误，入口旁显示红色"有错误"标签作为提示
- 展开后分为两个区域：
  - **最近连接错误**：以红色边框卡片展示最近一次数据库连接测试的详细错误信息（如网络超时、认证失败、驱动缺失等），仅在有错误时显示
  - **应用运行日志**：终端风格的黑色背景日志区域，展示应用运行期间的所有 INFO/WARN/ERROR 级别日志以及数据库模块的 DEBUG 日志，ERROR 级别以红色、WARN 以黄色、其余以灰色显示
- 日志面板提供「刷新」和「清空」两个操作按钮
- 后端使用内存环形缓冲区（最多 2000 条）存储日志，避免内存无限增长
