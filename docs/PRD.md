...
- 多数据库统一适配：在统一抽象层下支持 8 种数据库，包括 MySQL、PostgreSQL、openGauss、TXSQL、TDSQL、GaussDB、Oracle、达梦 DM
...
| 达梦 DM | ODBC | 5236 | 驱动随安装包打包分发，最终用户零安装；编译时需 unixODBC 头文件（`brew install unixodbc` / `apt install unixodbc-dev`） |
...
| 构建 | `make build-macos` / `build-macos-arm64` / `build-macos-x64` | 构建 macOS 通用二进制 / Apple Silicon / Intel |
| 构建 | `make build-windows` / `build-linux` | 构建 Windows MSVC / Linux GNU 包（需对应平台或 CI） |
| 构建 | `make build-all` | 在当前机器构建所有可用平台（默认 macOS arm64 + x64） |
...
- 达梦 DM 适配器基于 odbc-api（ODBC）实现；应用会随安装包携带各平台达梦 ODBC 驱动二进制实现零安装，运行时通过可执行文件相对路径定位驱动；驱动未随包时回退到系统注册的 `DM8 ODBC DRIVER` 名称；编译时仍需 unixODBC 头文件（macOS `brew install unixodbc`、Linux `apt install unixodbc-dev`）。Oracle 适配器基于 oracle-rs 0.1（纯 Rust TNS 协议）默认参与编译，零系统依赖运行
