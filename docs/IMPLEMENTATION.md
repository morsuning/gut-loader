...
- openGauss / GaussDB 复用 PG 适配器但使用专门的兼容连接参数：去除 `sqlx` 默认启动参数 `extra_float_digits`，并在当前无证书配置模型下显式禁用 TLS，避免 Windows 环境中 `sqlx` 默认 `sslmode=prefer` 进入 `rustls` 客户端证书协商链路后失败；原生 PostgreSQL 保持 `sqlx` 默认 TLS 策略
...
| `test_connection` | `config: DatabaseConfig` | `bool` / `Err(详情)` | 调用 `create_loader` 后 `test_connection`，失败时返回详细错误信息 |
...
| `get_app_logs` | — | `Vec<String>` | 从内存环形缓冲区读取应用运行日志 |
| `clear_app_logs` | — | `()` | 清空日志缓冲区 |
...
路径：`src-tauri/src/log_buffer.rs`

实现自定义 `tracing::Layer`，将应用运行日志捕获到内存环形缓冲区，供前端调试面板查询。

```mermaid
graph LR
    App[应用各模块 tracing 宏] --> Fmt[fmt::layer 输出到 stderr]
    App --> Buf[BufferLayer 写入环形缓冲区]
    Buf --> Ring[VecDeque<LogEntry> 最多 2000 条]
    Front[前端 get_app_logs] --> Ring
```

核心设计：

- `LogEntry` 结构体包含 timestamp、level、target、message 四个字段
- 全局使用 `OnceLock<Arc<RwLock<VecDeque<LogEntry>>>>` 存储，线程安全且延迟初始化
- 过滤策略：捕获 INFO 及以上级别的所有日志，以及数据库模块（`gut_loader_lib::database`）的 DEBUG 日志，避免高频调试日志（如批量插入进度）占用过多内存
- `get_app_logs` Tauri 命令将缓冲区内容格式化为 `[时间戳] [级别] target: message` 字符串列表返回给前端
- `clear_app_logs` 命令清空缓冲区

`lib.rs` 初始化时通过 `tracing_subscriber::registry()` 组合 `fmt::layer`（输出到 stderr）与 `BufferLayer`（写入内存），两者并行接收所有日志事件。
...
| `components/DatabaseConfig.tsx` | 步骤 02：数据库参数表单、连接测试、调试日志面板 |
...
- 主流程为单页向导，不引入路由层；报告页通过 `React.lazy` + `Suspense` 延迟加载，使首屏无需解析 recharts 图表代码
...
不同操作系统的磁盘空间 API 差异较大（macOS 使用 statfs、Linux 使用 statvfs、Windows 使用 GetDiskFreeSpaceEx），需要针对不同平台采用不同策略。

解决方案：

- 通过条件编译（`#[cfg]`）实现平台分发：`get_available_space` 作为入口函数，根据 `target_os` 调用对应实现
- **macOS / Linux**：通过 `std::process::Command` 执行系统 `df -k` 命令并解析输出第二行第 4 列（Available）
- **Windows**：通过 `windows-sys` crate 调用 `GetDiskFreeSpaceExW` API，直接获取可用空间字节数，无需子进程和输出解析
- 路径不存在时回退至父目录，避免命令/API 调用失败
...
Windows 窗口配置位于 `src-tauri/tauri.conf.json`。应用启动时先生成 Tauri context，再通过 `configure_windows_webview_args()` 按平台动态调整窗口配置；Windows 下使用 `EnumDisplayDevicesW` 枚举已连接到桌面的显示适配器，仅当全部适配器都命中软件/虚拟特征（Microsoft Basic Display、远程显示、RDP、VMware、VirtualBox、Parallels、Hyper-V、QXL、VirtIO，或 `DISPLAY_DEVICE_REMOTE` / `DISPLAY_DEVICE_MIRRORING_DRIVER` / `DISPLAY_DEVICE_RDPUDD` 标志）时，才向 WebView2 注入 `--disable-gpu`。真实 GPU 或探测失败时不写 `additionalBrowserArgs`，让 WebView2 保持默认硬件加速路径。
...
- 清理：`clean`、`clean-dist`、`clean-rust`、`rebuild`
- 帮助：`help`（默认目标）
...
- `src-tauri/Cargo.toml` 显式配置 release profile：`opt-level = 3`、fat LTO、单 codegen unit、符号裁剪与 `panic = "abort"`，确保发布构建启用完整优化
...
- `build-linux-x64` 在 macOS 上通过 `Dockerfile.linux-build` 构建 Ubuntu 24.04 Linux 环境，并以 `x86_64-unknown-linux-gnu` 目标执行 release 构建；镜像内安装 WebKitGTK 4.1 与 `javascriptcoregtk-4.1.pc`，满足 Tauri 2 的 Linux WebView 依赖
- `build-linux-arm` 在 macOS 上通过同一 Dockerfile 的 `linux/arm64` 镜像构建，并以 `aarch64-unknown-linux-gnu` 目标执行 release 构建；Linux 主机上仍可直接使用对应 GNU target 本地构建
- 构建目标完成后通过 `collect_release` / `collect_host_release` 将可执行文件与安装包复制到项目根目录 `dist/`；裸二进制自动追加 target 后缀避免平铺目录中同名覆盖，安装包保持 Tauri 原始文件名；`build-all` 先执行 `clean-dist`，确保最终发布目录只包含本轮构建产物
- `clean` 同时删除 `frontend/node_modules`、`frontend/dist`、`src-tauri/target`、`dist`，并执行 `cargo clean`，确保彻底回到初始状态
